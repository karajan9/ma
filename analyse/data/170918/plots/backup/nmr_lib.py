...
def fit_kohlrausch(time, cmplx, sigma=None, p0=[2500, 0.001, 1.1, 0]):
    p = curve_fit(kohlrausch, time, cmplx.real, p0=p0, sigma=sigma, absolute_sigma=True)
    p_value, p_cov = p
    p_error = np.sqrt(np.diag(p_cov))
    fit = kohlrausch(np.sort(time), p_value[0], p_value[1], p_value[2], p_value[3])
    return p_value, p_error, fit


# 
def fit_spek(time, data, sigma=None, p0=[2500, 0.001, 1.1, 0],
             bounds=([35e3, 0.0, 0.0, 0.0], [45e3, 1e2, 20.0, 2e3])):
    p = curve_fit(kohlrausch, time, data, p0=p0, sigma=sigma, absolute_sigma=True,
                                            bounds=bounds)
    p_value, p_cov = p
    p_error = np.sqrt(np.diag(p_cov))
    fit = kohlrausch(np.sort(time), p_value[0], p_value[1], p_value[2], p_value[3])
