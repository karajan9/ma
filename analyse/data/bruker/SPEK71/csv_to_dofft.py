...
for subdir in subdirs[:]:
    try:
        ...
        for i in range(udic["ndim"]):
            for j, k in iter(udic[i].items()):
                #~ print "%s:%s" %(j,k)
                header_string = header_string + "%s:%s\n" % (j, k)
        adv_header_string = ""
        for j, k in iter(dic.items()):
            if type(dic[j]) == type({}):
                adv_header_string = adv_header_string + "%s:\n" % (j)
                for l, m in iter(dic[j].items()):
                    if type(dic[j][l]) == 'dict':
                        adv_header_string = adv_header_string + "%s:\n" % (l)
                        for p, q in iter(dic[j][l].items()):
                            #~ print "%s:%s" %(p,q)
        ...
        if udic["ndim"] == 1:
            ...
            np.savetxt(
                ...
                np.transpose(
                    np.array([
                        np.arange(0, len(data), 1) * 1. / (udic[0]['sw']),
                        data.real,
                        np.zeros(len(data)), data.imag,
                        np.zeros(len(data))
                    ])),
            ...
            phase = 180 / 180. * np.pi
            plt.plot(
                np.cos(phase) * data[-1].real + np.sin(phase) * data[-1].imag)
            plt.plot(-1 * np.sin(phase) * data[-1].real +
                     np.cos(phase) * data[-1].imag)
            plt.show()
            plt.clf()
            plt.plot(
                np.cos(phase) * np.mean(data[:, 72:76].real, axis=1) +
                np.sin(phase) * np.mean(data[:, 72:76].imag, axis=1), "o")
            plt.show()
            np.savetxt(
                "./%s/T1_data.txt" % (subdir),
                np.cos(phase) * np.mean(data[:, 72:76].real, axis=1) +
                np.sin(phase) * np.mean(data[:, 72:76].imag, axis=1),
                header=header_string)
            #~ print len(data)
