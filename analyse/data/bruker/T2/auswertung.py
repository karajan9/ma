...
dirnamet2 = [
    "13", "15", "18", "21", "24", "26", "29", "31", "34", "36", "39", "41",
    "44", "47", "50", "57"
]
tempt2 = np.array([
    380.0, 390.0, 375.0, 385.0, 370.0, 365.0, 360.0, 355.0, 350.0, 345.0,
...
trigger_time_t2 = 2e-6
t2phase = 12 * np.pi / 180
...
def extract_bruker_t2(folder, trigger_time, phase, T2, beta1):
    times_1, signal, times_2, echos, echo_err = extract_bruker_generic(
        folder, trigger_time, phase)
    ...
    startparams = [T2, maxv, minv, beta1]
    bounds = ([1e-7, 0.0, -1e4, 0.0], [1e-3, 1e6, 1e4, 2.0])
    # bounds = ([1e-7, maxv - np.abs(maxv)*0.5, minv - np.abs(minv)*0.5, 0.0],
    #           [1e-2, maxv + np.abs(maxv)*0.5, minv + np.abs(minv)*0.5, 2.0])
    hold = [False, False, False, False]
    ...
    np.savetxt(
        "auswertung/%s_T2.fit.dat" % (folder),
        np.array([
            fitcurvet, fitcurvem, fitcurvem / fitparams[1],
            (fitcurvem - fitparams[2]) / fitparams[1]
        ]).transpose(),
    ...
    fig.savefig("auswertung/%s_T2.plot.pdf" % (folder))
    plt.close(fig)
...
def do_t1():
    ...
    for fold in dirnamet1:
        fitparam, fiterrs = extract_bruker_t1(fold, trigger_time_t1, t1phase,
                                              1.0e-3, 1.0)
...
def do_t2():
    ...
    for i, fold in enumerate(dirnamet2):
        fitparam, fiterrs, phase = extract_bruker_t2(fold, trigger_time_t2, t2phase,
                                            1.0e-3, 1.0)
        T2.append(fitparam[0] * 2)
        T2err.append(fiterrs[0] * 2)
        beta1.append(fitparam[3])
        beta1err.append(fiterrs[3])

        print("{} {} {} {} {} {} {} {} {} {} {}".format(fold, tempt1[i], phase, fitparam[0]*2,
                    fiterrs[0]*2, fitparam[3], fiterrs[3], 0.0, 0.0, 0.0, 0.0))

    T2 = np.array(T2)
    T2err = np.array(T2err)
    beta1 = np.array(beta1)
    beta1err = np.array(beta1err)
    # tempt2 = np.array(tempt2)

    fig = plt.figure(figsize=(32 / 2.54, 18 / 2.54))

    ax1 = fig.add_subplot(1, 1, 1)
    ax1.errorbar(1000.0 / tempt2, T2, yerr=T2err, fmt="rx", label="T2")
    ax1.legend()
    ax1.set_yscale("log")
    ax1.set_ylim((1e-5, 1e-1))
