...
def f2_t1(x, M0, t1, beta, Moff):
    return M0 * np.exp(-(x / t1)**beta) + Moff


def f2(tm, tau, beta, Z, M0, Moff):
    return M0 * ((1 - Z) * np.exp(-(tm / tau)**beta) + Z
                 ) * np.exp(-(tm / t1)**beta_t1) + Moff
...
def load_data(directory):
    fn_nmr = glob.glob(directory + "/*.dat.nmr")[0]
...
def load_FWHMs(directory):
    ...
    for index, fn in enumerate(fn_spek):
        temp, fwhm, peak, mean = load_FWHM(fn)
...
def get_tau(directory):
    fn = sorted(glob.glob(directory + "/*.info"))[0]
    with open(fn, "r") as info_file:
        for line in info_file:
            if "Evolution time tau" in line:
                return float(line.split()[-1])
    return 0.0
...
def calc_fullwidth_xmax(x=0.5, spek_fn=None):
...
def calc_spek_diff(comparison_real, x=0.5, spek_fn=None):
    freq, real, imag = load_spek(spek_fn)

    # normieren
    real /= np.max(real)
    comparison_real /= np.max(comparison_real)

    spek_argmax = np.argmax(comparison_real)
    spek_max = comparison_real[spek_argmax]
    xmax = spek_max * x

    lower = np.argmin(np.abs(comparison_real[:spek_argmax] - xmax))
    higher = np.argmin(np.abs(comparison_real[spek_argmax:] - xmax))

    left = comparison_real[lower] - real[lower]
    right = comparison_real[spek_argmax + higher] - real[spek_argmax + higher]
    return left, right
...
def fit_t1(reptime, cmplx, sigma=None):
    p0 = [80, 1e-4, 0.9, 40]
    p = curve_fit(
        t1, reptime, cmplx.real, p0=p0, sigma=sigma, absolute_sigma=True)
    p_value, p_cov = p
...
def fit_t2(tau, cmplx, sigma=None):
    p0 = [2500, 0.001, 1.1, 0]
    p = curve_fit(
        t2, tau, np.abs(cmplx), p0=p0, sigma=sigma, absolute_sigma=True)
    p_value, p_cov = p
    p_error = np.sqrt(np.diag(p_cov))
...
def fit_t2_fest(tau, cmplx, sigma=None):
    p0 = [800, 3e-5]
    p = curve_fit(
        t2_fest, tau, np.abs(cmplx), p0=p0, sigma=sigma, absolute_sigma=True)
    p_value, p_cov = p
    p_error = np.sqrt(np.diag(p_cov))
    fit = t2(np.sort(tau), p_value[0], p_value[1], p_value[2], p_value[3])
    return p_value, p_error, fit
...
def fit_f2(tm, cmplx, sigma=None):
    ...
    p = curve_fit(
        f2, tm, cmplx.real, p0=startwerte, sigma=sigma, absolute_sigma=True)
    p_value, p_cov = p
    p_error = np.sqrt(np.diag(p_cov))
...
def fit_kohlrausch(time, cmplx, sigma=None, p0=[2500, 0.001, 1.1, 0]):
    p = curve_fit(
        kohlrausch, time, cmplx.real, p0=p0, sigma=sigma, absolute_sigma=True)
    p_value, p_cov = p
    p_error = np.sqrt(np.diag(p_cov))
    fit = kohlrausch(
        np.sort(time), p_value[0], p_value[1], p_value[2], p_value[3])
    return p_value, p_error, fit
...
def fit_spek(time,
             ...
             sigma=None,
             p0=[2500, 0.001, 1.1, 0],
             bounds=([35e3, 0.0, 0.0, 0.0], [45e3, 1e2, 20.0, 2e3])):
    p = curve_fit(
        kohlrausch,
        time,
        data,
...
def fit_gauss(freq, data, sigma=None):
    ...
    p = curve_fit(
        gauss, freq, data, p0=startwerte, sigma=sigma, absolute_sigma=True)
    p_value, p_cov = p
    p_error = np.sqrt(np.diag(p_cov))
...
def fit_lorentz(freq, data, sigma=None):
    ...
    p = curve_fit(
        lorentz, freq, data, p0=startwerte, sigma=sigma, absolute_sigma=True)
    p_value, p_cov = p
    p_error = np.sqrt(np.diag(p_cov))
    a, gamma, x0 = p_value
...
def scatter_data(time, data, label="", marker="x"):
    if np.iscomplexobj(data):
        plt.scatter(
            time, data.real, color="b", marker=marker, label=label + " real")
        plt.scatter(
            time, data.imag, color="r", marker=marker, label=label + " imag")
    else:
...
def save_plot(fig, fn=None):
    ...
    for file_type in file_types:
        fig.savefig(fn_save + "." + file_type, bbox_inches="tight")
