...
def analyze_data(directory, t1_data, index, verbose=True):
    ...
    # params["Moff"].set(value=0.0)
    result = f2t1_model.fit(cmplx.real, params, x=reptime, weights=1/real_err)
    ...
    if verbose:
        ...
        composed = f2_t1(reptime,
                        result.params["M0"].value,
                        # np.mean(cmplx.real[0:4]),
                        t1_data[index,3],
                        t1_data[index,5],
                        result.params["Moff"].value)
        plt.plot(reptime, cmplx.real, 'bo', label="real")
        # plt.plot(reptime, cmplx.imag, 'ro', label="imag")
        plt.plot(reptime, result.best_fit, 'g-', label="F2 Daten fit")
        plt.plot(reptime, composed, 'k-', label="M0/Moff von fit, $T_1$/$\\beta$ von $T_1$")
...
# %%
data_id = "170706"
data_dir = home_dir + "/data/" + data_id + "/F2COS"
